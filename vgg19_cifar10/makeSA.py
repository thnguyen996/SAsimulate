...
# Calculate stuck bits
    ...
    orig_indicies = stuck_bits_sum.argsort()
    srt_idx = orig_indicies.argsort()
    indx = cp.searchsorted(set_map, stuck_bits_sum[orig_indicies])
    error = cp.concatenate(cp.take(indx, srt_idx).nonzero())
    pdb.set_trace()
    ...
    if error.__len__() == 0:
        ...
        for error_idx in error:
            error_weight[error_idx, :] = orig_weight[error_idx, :]
        return cp.reshape(error_weight, orig_shape).view(cp.float32)
