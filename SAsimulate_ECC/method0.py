...
def main():
    ...
    writer = SummaryWriter('runs/{}-{}'.format(now, "ECC (50 points) 1e-10 -- 1e-05, 50"))
    # Create model
...
def method0(model, total_param, error_total):
    ...
    with torch.no_grad():
      for name, param in model.named_parameters():
        ...
        output = ECC(output, param_binary)
...
def ECC(error_tensor, original_tensor):
    ...
    error_bool = error_flatten > 0.
    original_bool = original_flatten > 0.
    stuck_bits = (error_bool ^ original_bool).float()
    for index in range(0, stuck_bits.numel(), 64):
        stuck_bits_64 = stuck_bits[index:index + 64]
        if torch.nonzero(stuck_bits_64, as_tuple=False).shape[0] == 0:
            continue
        else:
            error_index = torch.min(torch.nonzero(stuck_bits_64, as_tuple=False))
