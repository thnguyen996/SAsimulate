...
def bit2float(b, num_e_bits=8, num_m_bits=23, bias=127.0):
  ...
  """
    ...
    assert b.shape[-1] == expected_last_dim, (
        "Binary tensors last dimension "
        "should be {}, not {}.".format(expected_last_dim, b.shape[-1])
    )
    ...
    if expected_last_dim > 64:
        warnings.warn(
            "pytorch can not process floats larger than 64 bits, keep"
    ...
    e = torch.index_select(
        b, -1, torch.arange(1, 1 + num_e_bits).to(torch.device("cuda"))
    )
    m = torch.index_select(
        b,
        -1,
        torch.arange(1 + num_e_bits, 1 + num_e_bits + num_m_bits).to(
            torch.device("cuda")
        ),
    )
    # SIGN BIT
    ...
    # EXPONENT BIT
    exponents = -torch.arange(-(num_e_bits - 1.0), 1.0)
    exponents = exponents.repeat(b.shape[:-1] + (1,))
    e_decimal = torch.sum(e * 2 ** exponents.to(torch.device("cuda")), dim=-1) - bias
    out *= 2 ** e_decimal
    # MANTISSA
    matissa = (torch.Tensor([2.0]) ** (-torch.arange(1.0, num_m_bits + 1.0))).repeat(
        m.shape[:-1] + (1,)
    )
    out *= 1.0 + torch.sum(m * matissa.to(torch.device("cuda")), dim=-1)
    return out


def float2bit(f, num_e_bits=8, num_m_bits=23, bias=127.0, dtype=torch.float32):
...
def bit2integer(b, num_bits=8):
    s = torch.index_select(b, -1, torch.arange(0, 1).to("cuda"))
    remain = torch.index_select(b, -1, torch.arange(1, num_bits).to("cuda"))

    sign = ((-1) ** s).squeeze(-1).type(torch.float32)

    exponents = -torch.arange(-(num_bits - 1.0), 1.0)
    exponents = exponents.repeat(b.shape[:-1] + (1,))
    decimal = torch.sum(b * 2 ** exponents.to(torch.device("cuda")), dim=-1) 
    negative = 2**num_bits - decimal
    negative = negative*sign
    out = torch.where(sign>0., decimal, negative)
...
if __name__ == "__main__":
    quantized = torch.tensor([
        [[-3.0, -6.0, -1.0], [-6.0, -10.0, -1.0], [9.0, 9.0, 6.0]],
        [[-4.0, -7.0, -3.0], [-4.0, -6.0, -1.0], [14.0, 16.0, 8.0]],
        [[-4.0, -6.0, -2.0], [-6.0, -9.0, -4.0], [9.0, 10.0, 5.0]],
    ], device="cuda")
    binary = integer2bit(quantized)
    bit2integer(binary)
